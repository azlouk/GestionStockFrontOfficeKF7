...
@Component({
  ...
  styleUrl: './facture-achat.component.scss'
})
export class FactureAchatComponent implements OnInit{

  initTabFacture: FactureAchat[]=[];
  facturesPage: Page<FactureAchat>={
  ...
  factures: Facture[] = [];
  Facturefilred: FactureAchat[] = [];
  FacturefilredSuplim: FactureAchat[] = [];
...
///p-dialogue deleteFactureAnd change prix
  ...
  constructor(
      ...
      public factureAchatService: FactureAchatService,
  ...
  loadFacture(page: number, size: number) {
    ...
    this.factureAchatService.LoadFactures(page, size).subscribe(
        (data: Page<FactureAchat>) => {
  ...
  editFacture(id: number) {
    this.router.navigate(['uikit/update-factureAchat/', id]);
  }

  addFacture() {
    this.router.navigate(['/uikit/add-factureAchat']);
  }
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        if(facture.typeFacture==factureType.ENTREE)
        {
          ...
          this.factureAchatService.deleteFacture(facture.id).subscribe(value => {
            this.loadFacture(this.currentPage,this.pageSize)
          })
        }
      },
  ...
  confirmDeleteFacture(factureAchat:FactureAchat) {
    this.factureAchatService.removeFactureWithUpdateProduct(factureAchat).subscribe(
        (response) => {

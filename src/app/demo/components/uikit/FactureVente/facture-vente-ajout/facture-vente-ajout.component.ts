...
@Component({
  selector: 'app-facture-vente-ajout',
  standalone: true,
  imports: [
    AjoutUserComponent,
    ButtonModule,
  ...
  templateUrl: './facture-vente-ajout.component.html',
  styleUrl: './facture-vente-ajout.component.scss'
})
export class FactureVenteAjoutComponent implements OnInit{

  @Input() showRetourButton: boolean = true;
  newFacture = new FactureVente();
  depots: Depot[] = [];
  produits: Produit[] = [];
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      if(this.fuctureId){
        this.factureVenteService.getFactureById(this.fuctureId).subscribe(value => {
  ...
  private saveFacture() {
    ...
    if (this.newFacture.id) {
      ...
      this.factureVenteService.updateFacture(this.newFacture).subscribe(
          (response) => {
      ...
      this.factureVenteService.addFacture(this.newFacture).subscribe(
          (response) => {
            this.messageService.add({
              severity: 'success',
              summary: 'Enregistrement',
  ...
  private resetForm() {
    ...
    this.newFacture = new FactureVente();
  }
  ...
  selectedFacture: FactureVente = new FactureVente();
